...
Для демо-версии polling каждые 2 секунды достаточен. Упрощает деплой — не нужны sticky sessions. В интерфейсе polling оптимизирован: обновляются только данные текущего экрана (активные аукционы/детали аукциона/инвентарь/история). В production заменяется на Socket.IO без изменения архитектуры.
