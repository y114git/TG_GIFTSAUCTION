...
```bash
cd backend
npx ts-node --transpile-only src/scripts/bot_swarm.ts 50
```

Боты делают действия с задержками на уровне каждого бота (поведение одного бота не зависит от общего числа ботов). В основной части раунда (>30s) они ставят агрессивно; в последние 15 секунд (anti-sniping окно) активность сильно снижена, чтобы не зацикливать аукцион.

Можно явно задать `concurrency` вторым аргументом:

```bash
cd backend
npx ts-node --transpile-only src/scripts/bot_swarm.ts 500 2000
```

Советую создавая новый аукцион, задавать достаточно большое число для минимальных ставок.
